...
# Disable torch dynamo completely to avoid Windows/Triton compilation issues
...
llama_rag_path = f'data/generated/generated_jokes_llama.json'
qwen_rag_path = f'data/generated/rag/outputs_qwen_rag.jsonl'
output_eval_path = f'data/evaluated/final_eval.jsonl'
...
try:
    ...
    print("Loading Gemma with Unsloth optimization...")
...
common_ids = sorted(list(set(llama_data.keys()) & set(qwen_data.keys())))
...
print("\nFinal Results:")
print(f"Llama wins: {stats['Llama']}")
print(f"Qwen wins: {stats['Qwen']}")
print(f"Ties: {stats['Tie']}")
print(f"Errors: {stats['Error']}")

# Determine overall winner and save summary
overall_winner = "Llama" if stats['Llama'] > stats['Qwen'] else ("Qwen" if stats['Qwen'] > stats['Llama'] else "Tie")
summary = {
    "judge_model": "google/gemma-2-9b-it",
    "comparison": "Llama_RAG vs Qwen_RAG",
    "llama_wins": stats['Llama'],
    "qwen_wins": stats['Qwen'],
    "ties": stats['Tie'],
    "errors": stats['Error'],
    "total_evaluated": stats['Llama'] + stats['Qwen'] + stats['Tie'] + stats['Error'],
    "overall_winner": overall_winner
}

summary_file = f'data/evaluated/final_eval_summary.json'
with open(summary_file, 'w', encoding='utf-8') as f:
    json.dump(summary, f, indent=2, ensure_ascii=False)
print(f"\n📊 Summary saved to: {summary_file}")
print(f"🏆 Overall Winner (Gemma Judge): {overall_winner}")
